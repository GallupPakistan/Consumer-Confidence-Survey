...
def render(df):
    st.markdown("<div style='font-size:24px; font-weight:700; color:#ffffff; margin-bottom:10px;'>📊 Future Condition (EEC)</div>", unsafe_allow_html=True)
    st.markdown("<div style='color:#8b949e; margin-bottom:30px;'>Analysis of household expectations for the economy over the next 12 months.</div>", unsafe_allow_html=True)
    ...
    base_idx = "Expected Economic Conditions Index"
    df['Year'] = df['Month'].dt.year
    
    # 1. National Trend
    fig1 = go.Figure(go.Scatter(x=df["Month"], y=df[base_idx].round(2), mode='lines+markers', name="National EEC", line=dict(color="#d29922", width=3)))
    apply_layout(fig1, "1. National Future Expectations Trend")
    st.plotly_chart(fig1, use_container_width=True)

    # 2. 6-Month Rolling
    rolling = df[base_idx].rolling(6, min_periods=1).mean()
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=df["Month"], y=df[base_idx].round(2), mode='lines', line=dict(color="rgba(210,153,34,0.3)", width=1)))
    fig2.add_trace(go.Scatter(x=df["Month"], y=rolling.round(2), mode='lines', name="6M Moving Avg", line=dict(color="#d29922", width=3)))
    apply_layout(fig2, "2. Smoothed Future Expectations")
    ...
    apply_layout(fig3, "3. Yearly Average Future Expectations")
    ...
    apply_layout(fig4, "4. Geographic Compare: Future Expectations")
    ...
    apply_layout(fig5, "5. Gender Compare: Future Expectations")
    ...
    apply_layout(fig6, "6. Education Compare: Future Expectations")
    ...
    # 10. Distributions
    fig10 = go.Figure(go.Box(x=df['Year'].astype(str), y=df[base_idx], boxpoints='all', marker_color="#d29922"))
    ...
    if vals:
        ...
        apply_layout(fig11, f"11. Future Expectations Segment Ranking")
