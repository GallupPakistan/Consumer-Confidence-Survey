...
def render(df):
    st.markdown("<div style='font-size:24px; font-weight:700; color:#ffffff; margin-bottom:10px;'>📊 Inflation Expectation (IE)</div>", unsafe_allow_html=True)
    st.markdown("<div style='color:#8b949e; margin-bottom:30px;'>Analysis of household expectations regarding price changes. <b>Note:</b> A higher DI indicates fear of rising prices.</div>", unsafe_allow_html=True)
    ...
    base_idx = "Inflation Expectation"
    df['Year'] = df['Month'].dt.year
    
    # 1. National Trend
    fig1 = go.Figure(go.Scatter(x=df["Month"], y=df[base_idx].round(2), mode='lines+markers', name="National IE", line=dict(color="#f85149", width=3)))
    apply_layout(fig1, "1. National Inflation Fear Trend")
    st.plotly_chart(fig1, use_container_width=True)

    # 2. 6-Month Rolling
    rolling = df[base_idx].rolling(6, min_periods=1).mean()
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=df["Month"], y=df[base_idx].round(2), mode='lines', line=dict(color="rgba(248,81,73,0.3)", width=1)))
    fig2.add_trace(go.Scatter(x=df["Month"], y=rolling.round(2), mode='lines', name="6M Moving Avg", line=dict(color="#f85149", width=3)))
    apply_layout(fig2, "2. Smoothed Inflation Expectations")
    st.plotly_chart(fig2, use_container_width=True)

    # 3. Annual Averages (Inverted Color Logic: High = Red)
    yearly = df.groupby('Year')[base_idx].mean().reset_index()
    fig3 = go.Figure(go.Bar(x=yearly['Year'].astype(str), y=yearly[base_idx].round(2), marker_color=["#f85149" if v>=50 else "#3fb950" for v in yearly[base_idx]], text=yearly[base_idx].round(1), textposition='auto'))
    apply_layout(fig3, "3. Yearly Average Inflation Expectations")
    ...
    apply_layout(fig4, "4. Geographic Compare: Inflation Expectations")
    ...
    apply_layout(fig5, "5. Gender Compare: Inflation Expectations")
    ...
    apply_layout(fig6, "6. Education Compare: Inflation Expectations")
    ...
    # 10. Distributions
    fig10 = go.Figure(go.Box(x=df['Year'].astype(str), y=df[base_idx], boxpoints='all', marker_color="#f85149"))
    apply_layout(fig10, "10. Boxplot Distributions by Year")
    st.plotly_chart(fig10, use_container_width=True)

    # 11. Snapshot (Inverted logic for IE: High = Red)
    lat = df.iloc[-1]
    lbls, vals = [], []
    for c in df.columns:
    ...
    if vals:
        ...
        s_vals, s_lbls = zip(*s)
        fig11 = go.Figure(go.Bar(x=s_vals, y=s_lbls, orientation='h', marker_color=["#f85149" if v>=50 else "#3fb950" for v in s_vals], text=[f"{v:.1f}" for v in s_vals], textposition='inside'))
        apply_layout(fig11, f"11. Inflation Fear Segment Ranking")
